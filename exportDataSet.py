...
def get_quicksight_client(region):
    ...
    return session.client('quicksight', verify=False, region_name=region)

#def get_dashboard_name(source_account_id, dashboard_id, region):
def get_dataset_name(source_account_id, dataset_id, region):
    quicksight_client = get_quicksight_client(region)
    print(source_account_id, " + ", dataset_id, " + ", region)
    try:
        ...
        print(f"Error retrieving dataset name line 31: {e}")
        return None, dataset_id

def start_export_job(account_id, region, export_bucket, dataset_id):
    quicksight_client = get_quicksight_client(region)
    ...
    try:
        response = quicksight_client.start_asset_bundle_export_job(
            ...
            ResourceArns=[f"arn:aws:quicksight:{region}:{account_id}:dataset/{dataset_id}"],
...
def monitor_export_job(account_id, region, export_job_id, dataset_name=None, dataset_id=None):
    quicksight_client = get_quicksight_client(region)
    try:
        while True:
            ...
            print(f"[Export Status] Dataset: {dataset_name} | ID: {dataset_id} | Status: {status}")
...
def main():
    parser = argparse.ArgumentParser(description='Export QuickSight dataset and get download URL.')
    ...
    print("Retrieving dataset name...")
    print(args.account_id, " + ", args.dataset_id, " + ", args.region)
    dataset_name, _ = get_dataset_name(args.account_id, args.dataset_id, args.region)
    if not dataset_name:
        print("Failed to retrieve dataset name.")
        return

    local_file = f"{dataset_name.replace(' ', '_')}.zip"

    print("Starting export job...")
    export_job_id = start_export_job(args.account_id, args.region, args.export_bucket, args.dataset_id)
    ...
    export_status, export_response = monitor_export_job(
        ...
        dataset_name=dataset_name, dataset_id=args.dataset_id
    )
