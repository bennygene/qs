...
def update_dataset_schema_with_env(data, env):
    ...
    # Compile a regex that matches any token from the three groups, plus CRCOMMRISKDB.
    pattern = re.compile(
        r'\b(CRANALYTICSDEV|CRANALYTICSCAT|CRANALYTICS|'
        r'CRRISKDBDEV|CRRISKCATDB|CRRISKDEVDB|CRRISKDB|'
        r'CRDATAHUBDEV|CRDATAHUBCAT|CRDATAHUB|CRCOMMRISKDB)\b',
        flags=re.IGNORECASE
    )
    
    def replacement(match):
        token = match.group(0).upper()
        # CRCOMMRISKDB handling for CAT
        if token == "CRCOMMRISKDB":
            if env_lower == "cat":
                return "CRCOMMRISKCATDB"
            elif env_lower == "dev" or env_lower == "qa":
                return "CRCOMMRISKDEVDB"
            else:
                return token
