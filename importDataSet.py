...
def clean_override_parameters(override_parameters):
    ...
    if 'DataSets' in new_override:
        cleaned_data_sets = []
        for ds in new_override['DataSets']:
            if isinstance(ds, dict):
                cleaned_ds = {k: v for k, v in ds.items() if k != "Type" and v is not None}
                cleaned_data_sets.append(cleaned_ds)
        new_override['DataSets'] = cleaned_data_sets
    return new_override
...
def monitor_import_job(account_id, region, job_id, dataset_name=None, dataset_id=None, profile=None):
    client = get_quicksight_client(region, profile)
    try:
        while True:
            ...
            print(f"[Import Status] Dataset: {dataset_name} | ID: {dataset_id} | Status: {status}")
